...
import { conferenceCopy as copy, sessionsForView } from '../data/conference.js'
...
export default function AgendaPage({ filter, setFilter, favorites, toggleFavorite }) {
  ...
  const groups = useMemo(() => {
    if (!recorded) return groupByDate(sessionsForView(filter, favorites)).map(group => ({ label: formatEventDate(group.date), events: group.events }))
  ...
  return <>
    ...
    <div className="conference-content">
      ...
      {groups.length === 0 && <p className="conference-empty">{filter === 'mySchedule' ? copy.emptySchedule : copy.empty}</p>}
      {groups.map(group => <section key={group.label} className="conference-group"><h2>{group.label}</h2>{group.events.map(event => <EventCard key={event.id} event={event} recorded={recorded} isFavorited={favorites.has(event.id)} onToggleFavorite={toggleFavorite} onSelect={setSelectedEvent} />)}</section>)}
    </div>
    {selectedEvent && <EventDetail event={selectedEvent} recorded={recorded} isFavorited={favorites.has(selectedEvent.id)} onToggleFavorite={toggleFavorite} onClose={() => setSelectedEvent(null)} />}
